...
namespace HoyoToon.HoyoToonEditor
{
    public class Presets : AssetPostprocessor
    {
        ...
        const string FILE_NAME_CACHE = "HoyoToon/preset_cache.txt";
        const string FILE_NAME_KNOWN_MATERIALS = "HoyoToon/presets_known_materials.txt";
            ...
            if (IsMaterialSectionedPreset(material))
            {
                ...
                foreach (string header in headers)
                {
                    if (IsSectionPreset(material, header))
                    {
                        ...
                        {
                            //HoyoToonLogs.LogDebug($"AddPreset: {name} ({guid})");
        ...
        static void RemovePreset(string guid)
        {
            ...
            foreach (KeyValuePair<string, PresetsCollection> collection in PresetCollections)
            {
                ...
                for (int i = 0; i < collection.Value.Guids.Count; i++)
                {
                    ...
                    {
                        //HoyoToonLogs.LogDebug($"RemovePreset: {collection.Value.Names[i]} ({guid})");
        ...
        public static void OpenPresetsMenu(Rect r, ShaderEditor shaderEditor, bool forceQuick, string collection = "_full_")
        {
            ...
            {
                HoyoToonLogs.LogDebug($"OpenPresetsMenu: {collection} ({PresetCollections[collection].Names.Count} presets)");
        ...
        public static void SetSectionPreset(Material m, string headerPropName, string name)
        {
            ...
            if (!string.IsNullOrWhiteSpace(name))
            {
                ...
                {
                    HoyoToonLogs.LogDebug($"SetSectionPreset: {name} ({guid})");
            ...
            else
            {
                if (PresetCollections.ContainsKey(headerPropName))
                {
                    if (PresetCollections[headerPropName].Guids.Contains(guid))
                    {
                        HoyoToonLogs.LogDebug($"RemovePreset: {PresetCollections[headerPropName].Names[PresetCollections[headerPropName].Guids.IndexOf(guid)]} ({guid})");
        ...
        static void CheckPresetCache()
        {
            ...
            if (cacheInvalid)
            {
                HoyoToonLogs.LogDebug("Preset cache invalid, rebuilding...");
                CreatePresetCache();
            }
        }
