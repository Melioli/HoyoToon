...
namespace HoyoToon.HoyoToonEditor.ShaderTranslations
{
    ...
    public partial class PropertyTranslation
    {
        ...
        public string GetAppropriateExpression(float value)
        {
            if (!UseConditionals)
            {
                HoyoToonLogs.LogDebug($"<b>{Origin}</b> -> <b>{Target}</b> doesn't use conditionals, directly returning the math expression <b>{Math}</b>");
                return Math;
            }
            ...
            foreach (ConditionalTranslationBlock block in ConditionalProperties)
            {
                if (block.ConditionType == ConditionalTranslationBlock.ConditionalBlockType.If)
                {
                    ...
                    if ((bool)result)
                    {
                        HoyoToonLogs.LogDebug($"<b>{Origin}</b> -> <b>{Target}</b>: <b>if</b> conditional <b>{block.ConditionalExpression}</b> returned math expression <b>{block.MathExpression}</b>");
                        return block.MathExpression;
                    }
                }
            }
