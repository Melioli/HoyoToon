...
namespace HoyoToon
{
    public class HoyoToonRGBAPackerDrawer : MaterialPropertyDrawer
    {
        ...
        Dictionary<UnityEngine.Object, HoyoToonRGBAPackerData> materialPackerData = new Dictionary<UnityEngine.Object, HoyoToonRGBAPackerData>();

        MaterialProperty _prop;
        HoyoToonRGBAPackerData _current;
        ...
        public HoyoToonRGBAPackerDrawer(string label1, string label2, string label3, string label4, string colorspace, string alphaIsTransparency)
        {
        ...
        public HoyoToonRGBAPackerDrawer(string label1, string label2, string colorspace, string alphaIsTransparency) : this(label1, label2, null, null, colorspace, alphaIsTransparency)
        {
        ...
        {
            _prop = prop;
            if (materialPackerData.ContainsKey(_prop.targets[0]) == false) materialPackerData[_prop.targets[0]] = new HoyoToonRGBAPackerData();
        ...
        void TexturePackerGUI()
        {
            ...
            if (changeCheck)
            {
                ...
                foreach (Material m in ShaderEditor.Active.Materials)
                    Undo.RegisterCompleteObjectUndo(m, "HoyoToon Packer Texture Change " + _prop.name);
                Save();
                Pack();
            }
        ...
        {
            if (undoRedoInfo.undoName == "HoyoToon Packer Texture Change " + _prop.name)
            {
