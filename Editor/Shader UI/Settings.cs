...
namespace HoyoToon
{
    ...
    {
        public const string MODULES_CONFIG = "HoyoToon/modules_config";
    ...
    public class Settings : EditorWindow
    {
        ...
        {
            base.Show();
            this.titleContent = new GUIContent("HoyoToon Settings");
        }
        ...
        public static ButtonData HoyoToon_message = null;
        ...
        private void InitVariables()
        {
            ...
            if (HoyoToon_message == null)
                WebHelper.DownloadStringASync(HoyoToon.URL.SETTINGS_MESSAGE_URL, (Action<string>)delegate (string s) { HoyoToon_message = Parser.Deserialize<ButtonData>(s); });
        }
        ...
        void OnGUI()
        {
            ...
            GUILayout.Label("HoyoToonEditor v" + Config.Singleton.verion);
        ...
        private void GUIMessage()
        {
            if (HoyoToon_message != null)
            {
                ...
                if (HoyoToon_message.text.Length > 0)
                {
                    doDrawLine = true;
                    GUILayout.Label(new GUIContent(HoyoToon_message.text, HoyoToon_message.hover), HoyoToon_message.center_position ? Styles.richtext_center : Styles.richtext);
                    Rect r = GUILayoutUtility.GetLastRect();
                    if (HoyoToon_message.action.type != DefineableActionType.NONE)
                        EditorGUIUtility.AddCursorRect(r, MouseCursor.Link);
                    if (Event.current.type == EventType.MouseDown && r.Contains(Event.current.mousePosition))
                        HoyoToon_message.action.Perform(ShaderEditor.Active?.Materials);
                }
                if (HoyoToon_message.texture != null)
                {
                    doDrawLine = true;
                    if (HoyoToon_message.center_position) GUILayout.Label(new GUIContent(HoyoToon_message.texture.loaded_texture, HoyoToon_message.hover), EditorStyles.centeredGreyMiniLabel, GUILayout.MaxHeight(HoyoToon_message.texture.height));
                    else GUILayout.Label(new GUIContent(HoyoToon_message.texture.loaded_texture, HoyoToon_message.hover), GUILayout.MaxHeight(HoyoToon_message.texture.height));
                    Rect r = GUILayoutUtility.GetLastRect();
                    if (HoyoToon_message.action.type != DefineableActionType.NONE)
                        EditorGUIUtility.AddCursorRect(r, MouseCursor.Link);
                    if (Event.current.type == EventType.MouseDown && r.Contains(Event.current.mousePosition))
        ...
        static Type s_vrchatAssetInstallerUIType { get; set; } = Helper.FindTypeByFullName("HoyoToon.VRChatAssetInstaller.VAI_UI");

        private void GUIVRChatAssetInstaller()
        {
            // check if HoyoToon.VRChatAssetInstaller.VAI_UI exists
            ...
            else
            {
                ...
                if (GUILayout.Button("Install VRChat Asset Installer"))
                {
                    ...
                    Client.Add("https://github.com/HoyoToonrallo/VRChat-Assets-Installer.git");
                }
