...
namespace HoyoToon
{
    ...
    {
        [MenuItem("HoyoToon/ShaderUI/Test/Custom Test")]
        ...
        [MenuItem("HoyoToon/ShaderUI/Test/Run Unit Tests")]
        public static void RunUnitTests()
        {
            ...
            {
                HoyoToonLogs.LogDebug($"Running test {test.t.Name}");
                object obj = null;
                object obj2 = null;
                ...
                {
                    HoyoToonLogs.ErrorDebug($"Failed to deserialize {test.t.Name} with error {e.Message}");
                    continue;
                }
                bool passed = serialized1 == serialized2 && serialized1 != null;
                HoyoToonLogs.LogDebug($"Serialization of {test.t.Name} failed. Serialized1: {serialized1} Serialized2: {serialized2}");
                passedTests += passed ? 1 : 0;
                testCount++;
            }
            if (testCount == passedTests)
            {
                HoyoToonLogs.LogDebug($"<color=#00ff00ff>Passed all tests</color>");
            }
            else
            {
                HoyoToonLogs.LogDebug($"<color=#ff7f00ff>Passed {passedTests}/{testCount} tests</color>");
            }
        }
        ...
        static List<(Type, string)> GetParserTests()
        {
            ...
            foreach (string line in txt.text.Replace("\r", "").Split('\n'))
            {
                if (line.StartsWith("##"))
                {
                    ...
                    if (type == null)
                    {
                        HoyoToonLogs.ErrorDebug($"Could not find type {className}");
                        continue;
                    }
                    tests.Add((type, ""));
                }
