...
namespace HoyoToon
{
    public class Parser
    {
        ...
        private static T DeserializeInternal<T>(string s)
        {
            ...
            {
                HoyoToonLogs.WarningDebug(e.ToString());
                HoyoToonLogs.WarningDebug(s + " cannot be parsed to object of type " + typeof(T).ToString());
        ...
        private static object DeserializeInternal(string s, Type t)
        {
            ...
            {
                HoyoToonLogs.WarningDebug(e.ToString());
                HoyoToonLogs.WarningDebug(s + " cannot be parsed to object of type " + t.ToString());
        ...
        private static object ParseJsonPart(string input, int start, int end)
        {
            ...
            if (input[start] == '{')
            {
                ...
                {
                    HoyoToonLogs.WarningDebug("Invalid json object: " + input.Substring(rawStart, rawEnd - rawStart));
                    return null;
                }
            }
            if (input[start] == '[')
            {
                ...
                {
                    HoyoToonLogs.WarningDebug("Invalid json array: " + input);
                    return null;
                }
            }
        ...
        {
            // HoyoToonLogs.LogDebug("Parse Object: "+ input.Substring(start, end - start));
            int depth = 0;
        ...
        {
            // HoyoToonLogs.LogDebug("Parse Array: " + input.Substring(start, end - start));
            int depth = 0;
            int variableStart = start;
        ...
        {
            // HoyoToonLogs.LogDebug("Parse Primitive: " + input);
            // string
        ...
        {
            object returnObject;
            if (TryHoyoToonParser(parsed, objtype, out returnObject))
                return returnObject;
        ...
        private static bool TryHoyoToonParser(object parsed, Type objtype, out object returnObject)
        {
            returnObject = null;
            if (Helper.IsPrimitive(parsed.GetType()) == false) return false;
            MethodInfo method = objtype.GetMethod("ParseForHoyoToonParser", BindingFlags.Static | BindingFlags.NonPublic);
        ...
        {
            if (objtype.BaseType == typeof(System.Array) && parsed.GetType() == typeof(string) && objtype.GetElementType().GetMethod("ParseToArrayForHoyoToonParser", BindingFlags.Static | BindingFlags.NonPublic) != null)
                return objtype.GetElementType().GetMethod("ParseToArrayForHoyoToonParser", BindingFlags.Static | BindingFlags.NonPublic).Invoke(null, new object[] { parsed });
        ...
        private static object ConvertToEnum(object parsed, Type objtype)
        {
            ...
            HoyoToonLogs.WarningDebug("The specified enum for " + objtype.Name + " does not exist. Existing Values are: " + Converter.ArrayToString(Enum.GetValues(objtype)));
    ...
    public class AnimationParser
    {
        ...
        public static Animation Parse(string path)
        {
            ...
            HoyoToonLogs.LogDebug(Parser.Serialize(animation));
            return animation;
        }
    }
}
