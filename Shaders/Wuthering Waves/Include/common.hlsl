...
void pom(in float3 normal, in float3 tangent, in float3 bitangent, in float3 view, in float2 uv, inout float2 height_uv)
{
    ...
    if ( mLevel <= (float) 10 )
    {
        ...
        while ( stepIndex < nNumSteps ) 
        {
            ...
            cHeight = _EM.SampleGrad(sampler_EM, cOffset, dx, dy).y;
...
{   
    ...
    // fdotl = fdotl + degrees(_FaceMapRotateOffset);
    ...
    float facing = step(-.5f,fdotl);

    // apply rotation offset
    // shadow_step = smoothstep(max(_FaceMapRotateOffset, 0.0), min(_FaceMapRotateOffset + 1.0f, 1.0f), shadow_step);
    
  
    float facemap = _MaskTex.Sample(sampler_MaskTex, faceuv).w;
    // interpolate between sharp and smooth face shading
    shadow_step = smoothstep(shadow_step - (_SDFSmoothness), shadow_step + (_SDFSmoothness), facemap);
...
float base_shadow(float ndotl, float ao_map)
{
    ...
    area = smoothstep(_ShadowProcess, _ShadowProcess + _ShadowWidth, area);
    area = min(area, 1.f);
...
//--------------------------------------------------------------------------------------
// SHADOW COLOR
float4 shadow_color_base(float3 normal, float3 light, float2 uv, float shadow_mask, float skin_id, float ramp_mask)
{
    // first shadow terms : 
...
float3 shadow_color_hair(float3 normal, float3 light, float4 mask, float skin_id)
{
    // first shadow terms : 
    ...
    // return the shadow color 
    return lerp(shadow_color, 1.0, saturate(shadow)); 
    // return shadow; 
}
//--------------------------------------------------------------------------------------
// SPECULAR
float3 specular_base(float3 normal, float3 half_vector, float shadow_mask, float power, float spec)
{
...
float4 matcap_specular(float3 normal, float shadow, float spec_check, float3 spec, float3 diffuse, float skin_id)
{
    ...
    float4 matcap = _MatCapTex.Sample(sampler_MatCapTex, sphere_uv);
...
//--------------------------------------------------------------------------------------
// material funtions 
void material_basic(inout float3 color, inout float4 shadow, inout float3 specular, in float3 normal, in float3 light, in float3 half_vector, in float3 spec, in float2 uv, in float shadow_mask, in float2 skin_id, in float2 typemask)
{
    ...
    specular = specular_base(normal, half_vector, shadow_mask, power, spec);
    specular = saturate(specular);

    // get shadow color
    shadow = shadow_color_base(normal, light, uv, shadow_mask, skin_id.x, typemask.y);

    float4 matcap = matcap_specular(normal, shadow.w, spec_check, spec, color, skin_id.x);
    color.xyz = matcap_coloring(color.xyz, matcap, spec);
    
}

void material_tight(inout float3 color, inout float4 shadow, inout float3 specular, in float3 half_vector, in float3 light, in float3 normal, in float3 tangent, in float3 bitangent, in float3 ws_pos, in float2 uv, float2 bump, in float3 view)
{
    float3 aniso = specular_tight(normal, tangent, bitangent, half_vector, dot(normal, view), ws_pos.xyz, uv, bump);
    
    float3 aniso_specular = aniso.x * (color_fix(_AnistropyColor) + 0.1);
    float3 shift = saturate(sqrt(lerp((_StockingEdgeColor), (_StockingColor), aniso.y)));
    ...
    stocking_light = stocking_light.x * color_fix(_StockingLightColor);

    float3 stocking = stocking_light.x * color_fix(_StockingLightColor) + color;

    // color = _StockingIntensity * specular + color;
    color = stocking;
}

void material_face(inout float3 shadow, in float3 normal, in float3 light, in float2 uv, in float shadow_mask, in float skin_id, in float typemask)
{
    shadow = shadow_color_base(normal, light, uv, shadow_mask, skin_id, typemask).xyz;
}

void material_hair(inout float3 shadow, inout float3 specular, in float3 normal, in float3 light, in float3 half_vector, in float4 hair_mask, in float skin_id)
{   
    shadow = shadow_color_hair(normal, light, hair_mask, skin_id);
...
void material_eye(inout float3 color, inout float stencilmask, inout float3 shine, in float3 normal, in float3 tangent, in float3 bitangent, in float2 uv, in float3 view, in float4 vertexcolor)
{
    ...
    float mask = saturate(_EM.Sample(sampler_EM, uv).w);
    float3 eye = _MainTex.Sample(sampler_MainTex, eye_uv);
    float3 eye_base = _MainTex.Sample(sampler_MainTex, uv);
    float stencil_mask_a = _StencilMask.Sample(sampler_StencilMask, eye_uv).x;
    float stencil_mask_b = _StencilMask.Sample(sampler_StencilMask, uv).x;
    ...
    float3 highlight = _HeightLightMap.Sample(sampler_HeightLightMap, r5.xy).xyz;
    r5.xyz = highlight;
    ...
    float shine2 = _EM.Sample(sampler_EM, r7.xy).x;
