vs_out vs_base(vs_in v)
{
    ...
    #endif
    if((0.0 /*_FaceMaterial*/)) o.normal = 0.5f;
...
vs_out vs_edge(vs_in v)
{
    ...
    {
        if((0.0 /*_FaceMaterial*/)) // sigh is this even going to work in vr? 
        {
            float4 tmp0;
            float4 tmp1;
            float4 tmp2;
            ...
            tmp0.x = tmp0.x * (0.1 /*_OutlineScale*/);
        ...
        else
        {
            ...
            float fov_width = 1.0f / (rsqrt(abs(wv_pos.z / unity_CameraProjection._m11)));
            if(!(1.0 /*_EnableFOVWidth*/))fov_width = 1;
            wv_pos.xyz = wv_pos + (outline_normal * fov_width * (v.v_col.w * (0.1 /*_OutlineWidth*/) * (0.1 /*_OutlineScale*/)));
            o.pos = mul(UNITY_MATRIX_P, wv_pos);
        }
    }
...
float4 ps_base(vs_out i, bool vface : SV_IsFrontFace) : SV_Target
{
    ...
    float4 color = ((0.0 /*_HairMaterial*/)) ? float4(1,1,1,1) * float4(0.4160188,0.4534367,0.6070304,0.7647059) : float4(0.4160188,0.4534367,0.6070304,0.7647059);
    ...
    float4 lut_bloomval = _MaterialValuesPackLUT.Load(float4(material_ID, 7, 0, 0)); // x: rim shadow width, y: rim shadow feather 
        ...
        #endif
        #if defined(use_rimlight)
            if(!(0.0 /*_FaceMaterial*/)) out_color.xyz = lerp(out_color.xyz.xyz - rim_light.xyz, out_color.xyz + rim_light.xyz, rim_values[curr_region].z);
        #endif
        if(!(0.0 /*_IsTransparent*/) && !(0.0 /*_EnableAlphaCutoff*/)) out_color.w = 1.0f;
        if((1.0 /*_EyeShadowMat*/)) out_color = float4(0.4160188,0.4534367,0.6070304,0.7647059);
        ...
        if((0.0 /*_FaceMaterial*/)) normal = float3(0.5f, 0.5f, 1.0f);
        ...
        #if defined(faceishadow)
            if((0.0 /*_FaceMaterial*/)) shadow_area = dot((float3)0.5f, light);
        #endif
        ...
        if((1.0 /*_EnableStencil*/))
        {
            ...
            else if((0.0 /*_HairMaterial*/))
            {
                ...
                hair_alpha = ((1.0 /*_UseHairSideFade*/)) ? max(max(hair_alpha, hair_blend), 0.0f) : hair_blend;
                float side_mask = 1.0f;
                if((0 /*_HairSideChoose*/) == 1) side_mask = saturate(step(0, i.vertex.x));
                if((0 /*_HairSideChoose*/) == 2) side_mask = saturate(step(i.vertex.x, 0));
...
float4 ps_edge(vs_out i, bool vface : SV_IsFrontFace) : SV_Target
{
    ...
    if((0.0 /*_FaceMaterial*/)) out_color = float4(0,0,0,1);
    out_color.a = 1.0f;
    #if defined(can_shift)
    ...
    if((0.0 /*_EnableAlphaCutoff*/)) clip(alpha - (0.5 /*_AlphaCutoff*/));
    return out_color;
}
